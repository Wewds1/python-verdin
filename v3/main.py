...
import numpy as np
from config import Camera, ROI, videos, videos_dict, corner_threshold, snap_distance, server_ip


for cameras in videos:
    video_cap = cv2.VideoCapture(cameras.rtsp_link, cv2.CAP_FFMPEG)
...
register_mtx_path(server_ip, cameras.camera_name)
...
ffmpeg_cmd = [
    ...
    '-pix_fmt', 'yuv420p',
    '-f', 'rtsp',
    f'rtsp://{server_ip}:8554/live/{cameras.camera_name}' 
]
